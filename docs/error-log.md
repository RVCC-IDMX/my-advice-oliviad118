...
| 2026-05-08 | Loading spinner never disappears - CSS specificity bug | style.css:43 & 342 | `.hidden` and `.spinner` have equal specificity but `.spinner` comes later, so it wins | Switched to HTML `hidden` attribute (Strategy C) - changed `loadingSpinner.classList.add('hidden')` to `loadingSpinner.hidden = true` in app.js, and `class="spinner hidden"` to `class="spinner" hidden` in index.html | me - didn't understand CSS specificity and source order |
